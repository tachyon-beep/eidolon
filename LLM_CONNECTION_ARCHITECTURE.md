# MONAD LLM Connection Architecture

## Overview

MONAD connects to Anthropic's Claude API using the official `anthropic` Python SDK with comprehensive resilience patterns for production-grade reliability.
...
MONAD makes LLM calls at three hierarchical levels during code analysis:
...
monad_ai_api_calls_total{status="success",model="claude-3-5-sonnet-20241022"} 1523

# Total tokens consumed (input + output)
monad_ai_api_tokens_total{direction="input",model="..."} 2847392
monad_ai_api_tokens_total{direction="output",model="..."} 1523847

# API call latency distribution
monad_ai_api_latency_seconds{model="..."}  # Histogram

# Rate limiter wait times
monad_ai_api_rate_limit_wait_seconds  # How long we waited

# Rate limiter wait count
monad_ai_api_rate_limit_waits_total  # How many times we waited
```
...
MONAD is currently hardcoded to use Anthropic's Claude, but could be extended to support other providers:
...
Check: Metrics show monad_ai_api_rate_limit_waits_total
```
...
Check: Metrics show monad_circuit_breaker_state{name="ai_api"}=2
...
**MONAD → Anthropic Claude Connection**:
