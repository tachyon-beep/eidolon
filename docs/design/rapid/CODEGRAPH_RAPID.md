...
* Query benchmark (`eidolon-codegraph-query-bench`) now includes Q7 (top import targets), Q8 (boundary stats derived table), and Q9 (call frequency) thanks to enriched ingestion (functions/calls/boundaries). Baseline latencies remain 1–3 ms.
* Concurrent query bench (`eidolon-codegraph-query-concurrency --clients 8 --iterations 20`) against CPython run `id=2`: p95 latencies remain below 4 ms (even for Q7 imports/Q9 calls); see `benchmarks/reports/cpython_concurrency_run2.json`.
* Rename/move simulation: `eidolon-synthetic-rename --count 1000` moved 1k modules; follow-up scan ingested as run `id=1`. SHA-based comparison (`eidolon-codegraph-compare`) detected exactly 1 000 renamed artefacts, 10 001 unchanged, zero adds/removals (see `benchmarks/reports/rename_comparison.json`). Demonstrates SHA stability for rename detection prior to introducing formal artefact IDs.
* Real repo baseline: cloned CPython (`benchmarks/cpython`), rescanned with import capture and ingested as run `id=2`. Scan summary: 6 132 `.py` files (~1.4 M SLOC). Serialized queries stay under 3 ms (including import stats); concurrent bench with 8 clients shows p95 ≤1.6 ms for import-heavy query Q7 (same report as above).
